...
> （要検証：具体的にどの論文・どんな比較だったかを [[参考文献・論文メモ]] に残しておきたい）。
...
この理解が正しいかどうかは検証が必要（[[アルゴリズム・パイプライン設計]] で深掘り）。
...
> Z7-20との定量的な比較表を作る（[[FPGA実装]] 側で実施予定）。

---

## 6. 現時点でのTODO

- [ ] KV260の正式スペック・価格・入手性を確認し、Z7-20との比較表を確定させる → [[FPGA実装]]
- [ ] ロングリードアライメントの計算構造を整理し、回路化しやすい処理単位に分解する → [[アルゴリズム・パイプライン設計]]
- [ ] 実際にminimap2等でナノポアのロングリードアライメントを動かし、ボトルネックを自分の目で確認する → [[ベンチマーク・性能検証]]
- [ ] 「ロングリードがスプライシング検出に有利」の根拠論文を探して要約する → [[参考文献・論文メモ]]
- [ ] 「ロングリードアライメントが重い」の根拠論文を探して要約する → [[参考文献・論文メモ]]
- [ ] バイオインフォマティクスの基礎知識を対話形式で積み上げる → [[基礎学習]]
